...
namespace Pmfst_GameSDK
{
    ...
    public abstract class Sprite
    {
        ...
        //private
        private int x, y, width, heigth; //svojstvo
        ...
        /// <remarks>Sprite se može instancirati i uz pomoć metode NewSprite koja se nalazi u klasi Game: <see cref="Game.NewSprite"/>.</remarks>
        public Sprite(string spriteImage, int posX, int posY, int spriteWidth, int spriteHeigth)
        {
            CurrentCostume = new Bitmap(spriteImage);
            CostumeName = spriteImage;
        ...
        {
            var values = Enum.GetValues(typeof(DirectionsType));
            foreach (DirectionsType d in values)
            {
                if (d.ToString().ToLower() == heading.ToLower())
                {
                    SetHeading(d);
                    return;
                }
        ...
        public void SetHeading(string heading)   // Preopterecena metoda
        {
            switch (heading.ToLower())    // gledamo vrijednost parametra 
            {
                case "left": Turn(270); break;      // Postavljamo smjer
                case "right": Turn(90); break;      // na željenu vrijednost
                case "up": Turn(0); break;
                case "down": Turn(180); break;
                default: Turn(90); break;
            }
        ...
        {
            int d = this.Direction + degrees;
            d %= 360;
            Turn(d);
        }

        //postavlja novi smjer 0-360     
        /// <summary>
        ...
        /// </summary>
        ///<remarks>Radi isto kao i <see cref="Sprite.SetHeading(System.Int32)"/>.</remarks>
        /// <param name="newDirection">Broj stupnjeva.</param>
        private void Turn(int newDirection)
        {

            if (RotationStyle == RotationStylesType.AllAround.ToString())
            {
                int i = this.CostumeIndex;
                try
                {
                    PointF offset = new PointF(w2, h2);

                    Bitmap bb;
                    bb = RotateImage(this.Kopija, offset, newDirection);
                    this.CurrentCostume = bb;
                }
                catch (Exception ex)
                {
                    //string m = "There is no costume with index " + i + " in Costumes"
                    //    + "\nSprite must have CurrentCostume and Costumes[" + i + "]";
                    //string m = ex.ToString();
                    //MessageBox.Show(m);
                }
            }
            else if (RotationStyle == RotationStylesType.LeftRight.ToString())
            {
                if (this.Direction == (int)DirectionsType.up && newDirection == (int)DirectionsType.right)
                {
                    this.Direction = (int)DirectionsType.right;
                    return;
                }

                if (this.Direction == (int)DirectionsType.right && newDirection == (int)DirectionsType.left)
                {
                    for (int i = 0; i < this.Costumes.Count; i++)
                    {

                        //lock (this.Costumes[i])
                            this.Costumes[i].RotateFlip(RotateFlipType.RotateNoneFlipX);

                    }
                }
                else if (this.Direction == (int)DirectionsType.left && newDirection == (int)DirectionsType.right)
                {
                    for (int i = 0; i < this.Costumes.Count; i++)
                    {
                        //lock (this.Costumes[i])
        ...
        public void AddCostumes(params string[] files)
        {
